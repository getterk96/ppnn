...
def create_observation_data_glob():
    all_data = []
    all_times = []
    all_dates = sorted(os.listdir(config.observation_raw_dir))
    for date in all_dates:
        date_files = sorted(os.listdir(os.path.join(config.observation_raw_dir, date)))
        for t in [f'{date}{t:02}.000.nc' for t in [0, 12]]:
            ...
            if len(all_times) != 0 and all_times.__len__() % 100 == 0:
